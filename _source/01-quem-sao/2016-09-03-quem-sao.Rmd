...
vereadorimg2vector = function(vereadores_lidos){
  v = vereadores_lidos %>% 
    gather() %>% 
    select(-1) %>% 
    filter(!is.na(value)) 
  return(v[seq(1, NROW(v), 2),])
}

vereadores_eleitos %>% 
  vereadorimg2vector() %>% 
  write_csv("vereadores_2012_eleitos_img-vector.csv", col_names = F)
vereadores_suplentes %>% 
  vereadorimg2vector() %>% 
  write_csv("vereadores_2012_suplentes_img-vector.csv", col_names = F)
```
